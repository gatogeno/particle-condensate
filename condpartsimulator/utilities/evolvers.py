...
def storequantities(step, t, parameters, grids, fieldY, pot, potnc, timeline, totalmass, energy):
    if parameters[4]=='spin0':
        #Kinetic energy particle and condensate
        energy[0][1][step]=adf.kineticenergy(parameters, grids, fieldY, 0)
        energy[1][1][step]=adf.kineticenergy(parameters, grids, fieldY, 1)
        energy[2][1][step]=energy[0][1][step]+energy[1][1][step]
        #Potential energy particle and condensate
        energy[0][2][step]=adf.potentialenergy(parameters, grids, fieldY, potnc, 0)
        energy[1][2][step]=adf.potentialenergy(parameters, grids, fieldY, pot, 1)
        ...
        totalmass[0][step]=np.sum(fieldY.densitypar)*(grids[0]**parameters[0])
        totalmass[1][step]=np.sum(fieldY.density)*(grids[0]**parameters[0])
        totalmass[2][step]=totalmass[0][step]+totalmass[1][step]
    else:
        energy[1][step]=adf.kineticenergy(parameters, grids, fieldY,1)
        energy[2][step]=adf.potentialenergy(parameters, grids, fieldY, pot, 1)
        energy[0][step]=energy[1][step]+energy[2][step]
        totalmass[step]=np.sum(fieldY.density)*(grids[0]**parameters[0])
    timeline[step]=t
    #Saving the data
    adf.savingstep(step, parameters, grids, fieldY, pot, potnc, timeline, totalmass, energy)
...
def kickdrift(parameters, grids, fieldY, pot, potnc, factordt):
    ...
    if parameters[4]=='spin0':
        energy=np.zeros((3,3,parameters[9][1]+1))
        totalmass=np.zeros((3,parameters[9][1]+1))
    else:
        energy=np.zeros((3,parameters[9][1]+1))
        totalmass=np.zeros(parameters[9][1]+1)
    timeline=np.zeros(parameters[9][1]+1)
    ...
    storequantities(0, t, parameters, grids, fieldY, pot, potnc, timeline, totalmass, energy)
    
    #The process of evolution
    for h in range(1,parameters[9][1]+1):
        ...
        t=t+dt
        storequantities(h, t, parameters, grids, fieldY, pot, potnc, timeline, totalmass, energy)
        
        adf.progbar(h+1,parameters[9][1]+1)
