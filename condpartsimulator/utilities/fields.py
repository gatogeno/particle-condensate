...
class spin0:
    def __init__(self, parameters, grids):
        ...
        if parameters[6][1][0]!='None':
            if parameters[6][1][0]=='testing':
                for i in range(parameters[7][0]):
                    for j in range(parameters[0]):
                        self.fieldpar[0][i][j]=0.5#np.random.uniform(-parameters[1]/4.0,parameters[1]/4.0)*0
                        self.fieldpar[1][i][j]=np.sign(np.random.uniform(0,1))*50
            ...
            if parameters[6][1][0]=='random':
                for i in range(parameters[7][0]):
                    ...
                    if parameters[0]==1:
                        ...
                        self.fieldpar[1][i][0]=module*(int(np.random.random()*2)*2-1)
                    if parameters[0]==2:
                        self.fieldpar[0][i][0]=np.random.uniform(np.amin(grids[2]),np.amax(grids[2]))
            ...
            if parameters[6][1][0]=='maxwellian':
                for i in range(parameters[7][0]):
                    counterpos=0
                    countervel=0
                    if parameters[0]==1:
                        while counterpos==0:
                            modulepos=np.random.uniform(0,parameters[1]/2.0)
                            rndmnum=np.random.uniform(0,1)
                            dist=np.exp(-0.5*pow(modulepos,2)/pow(parameters[6][1][1],2))/np.sqrt(2*np.pi)
                            dist2=1/np.sqrt(2*np.pi)
                            if rndmnum<=dist/dist2:
                                self.fieldpar[0][i][0]=modulepos*(int(np.random.random()*2)*2-1)
                                counterpos=1
                        while countervel==0:
                            modulevel=np.random.uniform(0,10)
                            rndmnum=np.random.uniform(0,1)
                            dist=np.exp(-0.5*pow(modulevel,2))*np.sqrt(2*np.pi)*parameters[6][1][1]
                            dist2=np.sqrt(2*np.pi)
                            if rndmnum<=dist/dist2:
                                self.fieldpar[1][i][0]=modulevel*(int(np.random.random()*2)*2-1)
                                countervel=1
                    if parameters[0]==2:
                        while counterpos==0:
                            modulepos=np.random.uniform(0,parameters[1]/2.0)
                            ...
                            dist2=1/np.sqrt(2*np.pi)
                            if rndmnum<=dist/dist2:
                                self.fieldpar[0][i][0]=modulepos*np.cos(angleph)
                                self.fieldpar[0][i][1]=modulepos*np.sin(angleph)
                                counterpos=1
                        while countervel==0:
                            modulevel=np.random.uniform(0,10)
                            rndmnum=np.random.uniform(0,1)
                            ...
                            dist2=np.sqrt(2*np.pi)
                            if rndmnum<=dist/dist2:
                                self.fieldpar[1][i][0]=modulevel*np.cos(angleph)
                                self.fieldpar[1][i][1]=modulevel*np.sin(angleph)
                                countervel=1
                    if parameters[0]==3:
                        while counterpos==0:
                            modulepos=np.random.uniform(0,parameters[1]/2.0)
                            ...
                            dist2=1/np.sqrt(2*np.pi)
                            if rndmnum<=dist/dist2:
                                self.fieldpar[0][i][0]=modulepos*np.sin(angleth)*np.cos(angleph)
                                self.fieldpar[0][i][1]=modulepos*np.sin(angleth)*np.sin(angleph)
                                self.fieldpar[0][i][2]=modulepos*np.cos(angleth)
                                counterpos=1
                        while countervel==0:
                            modulevel=np.random.uniform(0,10)
                            rndmnum=np.random.uniform(0,1)
                            ...
                            dist2=np.sqrt(2*np.pi)
                            if rndmnum<=dist/dist2:
                                self.fieldpar[1][i][0]=modulevel*np.sin(angleth)*np.cos(angleph)
                                self.fieldpar[1][i][1]=modulevel*np.sin(angleth)*np.sin(angleph)
                                self.fieldpar[1][i][2]=modulevel*np.cos(angleth)
                                countervel=1            
        ...
        if parameters[6][0][0]=='testing':
            if parameters[0]==1:
                ...
                alpha=0.25
                t=0
                self.field=np.sqrt(alpha/(alpha+1.0j*t))*np.exp(-0.5*((grids[2]+x0-1.0j*alpha*k0)**2)/(alpha+1.0j*t))*np.exp(-0.5*alpha*k0**2)+np.sqrt(alpha/(alpha+1.0j*t))*np.exp(-0.5*((grids[2]-x0+1.0j*alpha*k0)**2)/(alpha+1.0j*t))*np.exp(-0.5*alpha*k0**2)
        ...
        if parameters[6][0][0]=='centralsoliton':
            if parameters[0]==1:
                self.field=np.sqrt(((parameters[3]*1024*pow(0.091,1.5))/(33*pow(np.pi,2)*pow(parameters[6][0][1],3)))/(pow(1+0.091*((pow(grids[2],2))/(pow(parameters[6][0][1],2))),8)))*np.exp(-1.0j*np.random.random((parameters[2])))
            if parameters[0]==2:
        ...
        if parameters[6][0][0]=='multiple-solitons':
            if parameters[0]==1:
                ...
                for i in range(int(parameters[6][0][2])):
                    self.field=self.field+np.sqrt(((parameters[3]*1024*pow(0.091,1.5))/(33*pow(np.pi,2)*pow(parameters[6][0][1],3)*parameters[6][0][2]))/(pow(1+0.091*((pow(grids[2]-np.random.uniform(-(parameters[1]/4.0), (parameters[1]/4.0)),2))/(pow(parameters[6][0][1],2))),8)))*np.exp(-1.0j*np.random.random((parameters[2])))
            if parameters[0]==2:
                self.field=np.zeros((parameters[2],parameters[2]), dtype=complex)
                for i in range(int(parameters[6][0][2])):
                    self.field=self.field+np.sqrt(((parameters[3]*1024*pow(0.091,1.5))/(33*pow(np.pi,2)*pow(parameters[6][0][1],3)*parameters[6][0][2]))/(pow(1+0.091*((pow(grids[2]-np.random.uniform(-(parameters[1]/4.0),(parameters[1]/4.0)),2)+pow(grids[4]-np.random.uniform(-(parameters[1]/4.0),(parameters[1]/4.0)),2))/(pow(parameters[6][0][1],2))),8)))*np.exp(-1.0j*np.random.random((parameters[2],parameters[2])))
            if parameters[0]==3:
                self.field=np.zeros((parameters[2],parameters[2],parameters[2]), dtype=complex)
                for i in range(int(parameters[6][0][2])):
                    self.field=self.field+np.sqrt(((parameters[3]*1024*pow(0.091,1.5))/(33*pow(np.pi,2)*pow(parameters[6][0][1],3)*parameters[6][0][2]))/(pow(1+0.091*((pow(grids[2]-np.random.uniform(-(parameters[1]/4.0),(parameters[1]/4.0)),2)+pow(grids[4]-np.random.uniform(-(parameters[1]/4.0),(parameters[1]/4.0)),2)+pow(grids[6]-np.random.uniform(-(parameters[1]/4.0),(parameters[1]/4.0)),2))/(pow(parameters[6][0][1],2))),8)))*np.exp(-1.0j*np.random.random((parameters[2],parameters[2],parameters[2])))
